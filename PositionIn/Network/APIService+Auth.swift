...
extension APIService {
        ...
        return sessionController.session().recoverWith { _ in
            return self.refreshToken().map { response in
                return response.accessToken
                    ...
                    trackGoogleAnalyticsEvent(AnalyticCategories.token, action: AnalyticActios.refreshFailed, label: error.localizedDescription)
            }
        }
    }
    ...
    func verifyPhone(phoneNumber: String, type: NSNumber) -> Future<Void, NSError> {
        trackGoogleAnalyticsEvent(AnalyticCategories.phoneVerification, action: AnalyticActios.phoneVerificationRequested)
        return verifyPhoneRequest(phoneNumber, type: type)
    }
    
    //Validate Code
    func verifyPhoneCode(phoneNumber: String, code: String) -> Future<Bool, NSError> {
        trackGoogleAnalyticsEvent(AnalyticCategories.phoneVerification, action: AnalyticActios.codeVerificationRequested)
