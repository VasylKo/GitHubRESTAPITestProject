        ...
        if let codeRowValue = codeRow?.value,
            let phoneNumber = self.phoneNumber {
                ...
                api().verifyPhoneCode(phoneNumber, code: codeString).onSuccess(callback: {[weak self] isExistingUser in
                    if isExistingUser {
                        trackEventToAnalytics("Auth", action: "Click", label: "SMS verification", value: NSNumber(int: 1))
                        ...
                        if let strongSelf = self {
                            trackEventToAnalytics("Auth", action: "Click", label: "SMS verification", value: NSNumber(int: 0))
